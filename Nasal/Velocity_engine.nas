...
var engine_update = func (i) {						# i = engine number
  ...
  if (getprop(eprop~"running") == 1) {					# Is engine running?
    ...
    var dT = time - timestamp;						# multiply by fuel flow, and add to current fuel-consumed
    timestamp = time;							# Nasal fuel routines will reset fuel-consumed after deducting fuel
    ...
    setprop("/engines/engine[0]/rpm-anim", getprop("/engines/engine[0]/rpm"));
  }
    ...
    if (rpm < 30) {
      setprop("/engines/engine[0]/rpm-anim", 0);
    }
    else {
      setprop("/engines/engine[0]/rpm-anim", rpm);
    }
